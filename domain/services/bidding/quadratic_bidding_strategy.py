...
class QuadraticBiddingStrategy(BiddingStrategy):
    ...
    async def get_bid_for_cost(
        ...
        item_loans = sorted(
            filter(
                lambda l: l.item.id == item.id,
                self.loan_repository.get_loans_for_library(library),
            ),
        ...
        for loan in item_loans:
            if loan.borrower.entity_id == beneficiary.entity_id:
        ...
        amount_bid = (
            amount_to_pay.multiply(1 / num_previous_loans)
