...
class SimpleTimeBasedFeeSchedule(FeeSchedule):
    ...
    def __init__(self, money_factory: MoneyFactory, default_item_cost: Money = None, num_years: int = 1):
        self.num_years = num_years
        self.money_factory = money_factory
        self.default_item_cost = default_item_cost if default_item_cost else money_factory.get_empty_money()

    def _base_cost(self, loan: Loan) -> Money:
        """Returns the item's purchase cost if available; otherwise, uses the default cost."""
        return loan.item.purchase_cost if loan.item.purchase_cost else self.default_item_cost
    ...
    def fees_for_overdue_item(self, loan: Loan) -> Money:
        ...
        if loan.date_returned and loan.due_date:
            time_overdue = (loan.date_returned - loan.due_date).total_seconds() * 1000  # Convert to milliseconds
