...
def admin(person_name):
    return Person(person_id=ID(value="1"), name=person_name, email=[])
...
def test_borrowed_item_no_longer_available(library, thing):
    borrower = Borrower(
        username="libraryMember", person=library.administrator, library=library, fees=[]
    )
...
def test_cannot_borrow_with_fees(library, thing):
    ...
    library.add_borrower(borrower)
    loan = Loan(id="loan", item=thing, borrower=borrower, due_date=get_due_date())
    borrower.apply_fee(
        LibraryFee(amount=USDMoney(amount=120), loan=loan, status=FeeStatus.OUTSTANDING)
    )
