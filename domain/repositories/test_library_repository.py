...
from domain.entities import Person, Borrower
from domain.entities.libraries import Library
from domain.value_items import PersonName, ID
...
def test_gets_libraries_via_person():
    ...
    person = Person(person_id=ID.parse("test"), name=person_name)
    ...
    mock_borrower2.person = Person(person_id=ID.parse("nope"), name=person_name)
