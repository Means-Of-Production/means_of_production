...
from domain.entities import ILibrary, SimpleLibrary, DistributedLibrary, Person
from domain.value_items.location.physical_area import PhysicalArea


class LibraryRepository(BaseInMemoryRepository[ILibrary]):
    def __init__(self, libraries: Iterable[ILibrary] = ()) -> None:
        super().__init__()

        for lib in libraries:
            self.add(lib)

    def get_id_from_entity(self, entity: ILibrary) -> str:
        return entity.entity_id if entity.entity_id else entity.name

    def get_libraries_person_can_use(self, person: Person) -> Generator[ILibrary, None, None]:
        for library in self.get_all():
            if any(borrower.person.entity_id == person.entity_id for borrower in library.borrowers):
                yield library

    def create(self, entity: ILibrary) -> ILibrary:
